...
public class Path {
    ...
    public XPathFragmentSpecification getXPathFragmentSpecification() {
        ...
        return getFirst().getGeneratingFactory().getXPathFragmentSpecification();
    }

    public Set<String> getXPathExpression() {
        final CaseHandler caseHandler = getXPathFragmentSpecification().getCaseHandler();
        final XPathExpressionBuilder builder = new XPathExpressionBuilder(caseHandler);	    
        return builder.makeXPaths(this);
    }
    ...
    public String toString() {
        ...
        while (current!=EMPTY_PATH) {
            final XPathStep step = current.step;
            result.append(print(getStart()) + '\u2192' + "[" + step.getShortName( ) + "]");
            current = current.next;
        }
        result.append(print(getEnd()));
